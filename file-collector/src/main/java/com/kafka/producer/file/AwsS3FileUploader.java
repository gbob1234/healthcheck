...
/**
 * Uploads files with AWS SDK v2, using PutObject for small files and multipart upload for large
 * files.
 *
 * <p>SHA-256 values sent to S3 are Base64 encoded as required by the S3 protocol. Kafka metadata
 * keeps the full-file checksum in hexadecimal form.
 */
public final class AwsS3FileUploader implements S3FileUploader {
        ...
        UrlConnectionHttpClient.builder()
            .buildWithDefaults(
                AttributeMap.builder()
                    .put(SdkHttpConfigurationOption.TRUST_ALL_CERTIFICATES, !config.tlsVerify)
                    .build());
    this.client =
        S3Client.builder()
  ...
  private UploadResult multipart(Path file, String key, long size) throws IOException {
    ...
    try {
      ...
      while (offset < size) {
        if (partNumber > 10000)
          throw new IOException("Multipart upload exceeds S3's 10,000 part limit");
        long length = Math.min(config.multipartPartSizeBytes, size - offset);
        // A multipart checksum covers this part only, never the entire source file.
        ...
        UploadPartRequest request =
            UploadPartRequest.builder()
                ...
                .checksumSHA256(partChecksumSha256)
                .build();
        UploadPartResponse response =
            client.uploadPart(
                request,
                RequestBody.fromContentProvider(
                    new RangedFileProvider(file, offset, length),
                    length,
  ...
  private static String hexSha256ToBase64(String hex) {
    if (hex == null || hex.length() != 64)
      throw new IllegalArgumentException("SHA-256 hex value must contain 64 characters");
    byte[] bytes = new byte[32];
    for (int i = 0; i < bytes.length; i++) {
      int high = Character.digit(hex.charAt(i * 2), 16);
      int low = Character.digit(hex.charAt(i * 2 + 1), 16);
      if (high < 0 || low < 0)
        throw new IllegalArgumentException("SHA-256 contains a non-hex character");
  ...
  private static final class RangedFileProvider
      implements software.amazon.awssdk.http.ContentStreamProvider {
    private final Path file;
    private final long offset, length;

    RangedFileProvider(Path file, long offset, long length) {
      this.file = file;
      this.offset = offset;
      this.length = length;
    }
    ...
    public InputStream newStream() {
      try {
        ...
        while (remaining > 0) {
          ...
          if (skipped <= 0) {
            in.close();
            throw new IOException("Unable to seek multipart source");
          }
          remaining -= skipped;
        }
        return new LimitedInputStream(in, length);
      } catch (IOException e) {
        throw new IllegalStateException("Unable to open multipart source", e);
      }
    }
  }
