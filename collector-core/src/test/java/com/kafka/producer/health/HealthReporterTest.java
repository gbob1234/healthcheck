...
class HealthReporterTest {
    ...
    @Test void isIdempotentAndCallsFatalOnlyAfterCountAndDurationThresholds() throws Exception {
        ...
        HealthPublisher publisher = new HealthPublisher() {
            public void publish(String key, CloudEvent event, ResultCallback callback) { callback.complete(new RuntimeException("offline")); }
            public boolean reinitialize() { return false; }
            public void close() { }
        };
        FatalFailureHandler handler = (code, message, cause) -> fatal.countDown();
        HealthReporter reporter = new HealthReporter(config, state,
                new HealthStatusEvaluator(clock, 180), new HealthCloudEventFactory(config, new ObjectMapper(), clock),
                publisher, handler, clock);
