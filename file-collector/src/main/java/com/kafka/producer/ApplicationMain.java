...
public final class ApplicationMain {
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 1) {
            earlyLifecycle.terminate("INVALID_ARGUMENTS", "Usage: java -jar file-collector-2.0-SNAPSHOT.jar <config.properties>", null);
            return;
        }
    ...
    private static void start(Path configPath) throws Exception {
        ...
        try {
            ...
            FileDirectoryWatcher watcher = new FileDirectoryWatcher(config.fileCollector,
                    new FileStabilityChecker(config.fileCollector.stabilityIntervalMs,
            ...
            Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
                @Override public void run() { lifecycle.close(); }
            }, "application-shutdown-hook"));
            if (reporter != null) reporter.start();
            watcher.start();
        } catch (Exception e) { lifecycle.terminate("STARTUP_FAILED", e.getMessage(), e); }
    }
