...
public final class FileMetadataPublisher implements MetadataPublisher {
    ...
    FileMetadataPublisher(KafkaProducer<String, String> producer, String topic, ApplicationHealthState state) {
        this.producer = producer; this.topic = topic; this.state = state;
    }
    public void publish(String key, String json) throws Exception {
        state.fileSendAttempted();
        try {
            RecordMetadata metadata = producer.send(new ProducerRecord<String, String>(topic, key, json)).get();
            state.fileSendSucceeded();
            LOG.debug("File metadata sent: topic={}, partition={}, offset={}", metadata.topic(), metadata.partition(), metadata.offset());
