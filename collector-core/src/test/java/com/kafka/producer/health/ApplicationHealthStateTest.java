...
class ApplicationHealthStateTest {
  @Test
  void snapshotsCountersSequenceAndTruncatedErrorConcurrently() throws Exception {
    ApplicationHealthState state =
        new ApplicationHealthState(Clock.fixed(Instant.EPOCH, ZoneOffset.UTC));
    int threads = 4, iterations = 100;
    CountDownLatch done = new CountDownLatch(threads);
    for (int t = 0; t < threads; t++)
      new Thread(
              () -> {
                for (int i = 0; i < iterations; i++) state.fileDetected(i);
                done.countDown();
              })
          .start();
    done.await();
