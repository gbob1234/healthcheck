...
public final class HealthStatusEvaluator {
    ...
    public HealthStatus evaluate(HealthSnapshot s) {
        ...
        if (s.lastFileSendFailureAt != null && newer(s.lastFileSendFailureAt, s.lastFileSendSuccessAt))
            return new HealthStatus(HealthStatus.Level.WARN, "FILE_KAFKA_SEND_FAILED", "Latest file metadata delivery failed");
        if (s.lastFileProcessingFailureAt != null && newer(s.lastFileProcessingFailureAt, s.lastFileProcessingSuccessAt))
            return new HealthStatus(HealthStatus.Level.WARN, "FILE_PROCESSING_FAILED", "Latest file processing failed");
        if (s.lastWorkerProgressAt != null && Duration.between(s.lastWorkerProgressAt, now).getSeconds() > workerStaleSeconds)
            return new HealthStatus(HealthStatus.Level.WARN, "FILE_WATCHER_STALE", "File watcher has not reported progress");
