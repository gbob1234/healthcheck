...
public final class HealthKafkaPublisher implements HealthPublisher {
    ...
    private final AtomicReference<KafkaProducer<String, String>> producer = new AtomicReference<KafkaProducer<String, String>>();

    public HealthKafkaPublisher(Properties properties, String topic, ObjectMapper mapper) {
        this.properties = new Properties(); this.properties.putAll(properties);
        this.topic = topic; this.mapper = mapper;
    ...
    public void publish(String key, CloudEvent event, final HealthPublisher.ResultCallback callback) {
        try {
            ...
            producer.get().send(new ProducerRecord<String, String>(topic, key, json), new Callback() {
                @Override public void onCompletion(org.apache.kafka.clients.producer.RecordMetadata metadata, Exception exception) {
                    callback.complete(exception);
                }
            });
        } catch (Exception e) { callback.complete(e); }
    }

    public synchronized boolean reinitialize() {
        KafkaProducer<String, String> previous = producer.getAndSet(null);
        if (previous != null) try { previous.close(Duration.ofSeconds(3)); } catch (Exception ignored) { }
        try { producer.set(new KafkaProducer<String, String>(properties)); return true; }
