...
public final class KafkaPropertiesFactory {
  public Properties create(
      ApplicationConfig.KafkaCommon common, ApplicationConfig.Producer producer) {
    ...
    p.setProperty(
        ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG, String.valueOf(producer.requestTimeoutMs));
    p.setProperty(
        ProducerConfig.DELIVERY_TIMEOUT_MS_CONFIG, String.valueOf(producer.deliveryTimeoutMs));
  ...
  private void applySecurity(Properties p, ApplicationConfig.KafkaCommon c) {
    ...
    p.setProperty(
        SslConfigs.SSL_ENDPOINT_IDENTIFICATION_ALGORITHM_CONFIG, c.endpointIdentificationAlgorithm);
    ...
    if (c.securityMode == ApplicationConfig.SecurityMode.SASL_SSL) {
      ...
      p.setProperty(
          ...
          c.saslJaasConfig.isEmpty()
              ? createJaas(c.saslUsername, c.saslPassword)
              : c.saslJaasConfig);
    }
  }
