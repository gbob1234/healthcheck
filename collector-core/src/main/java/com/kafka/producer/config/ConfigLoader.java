...
public final class ConfigLoader {
  ...
  public ApplicationConfig load(Path path) throws IOException {
    ...
    if (!rootValue.isEmpty()) {
      ...
      if (archive.equals(watch))
        throw new IllegalArgumentException(
            "file.archive.directory must differ from file.watch.directory");
    ...
    if (accessKey.isEmpty() != secretKey.isEmpty())
      throw new IllegalArgumentException(
          "s3.access.key and s3.secret.key must be configured together");
    if (!sessionToken.isEmpty() && accessKey.isEmpty())
      throw new IllegalArgumentException(
          "s3.session.token requires s3.access.key and s3.secret.key");
    ...
    if (partSize < MIN_MULTIPART_PART_SIZE || partSize > MAX_MULTIPART_PART_SIZE)
      throw new IllegalArgumentException(
          "s3.multipart.part.size.bytes must be between 5242880 and 5368709120");
    if (maxFileSize > partSize * 10000L)
      throw new IllegalArgumentException(
          "file.max.file.size.bytes exceeds the configured 10,000 multipart part capacity");
    ApplicationConfig.S3 s3 =
        new ApplicationConfig.S3(
            endpoint,
            required(p, "s3.region"),
            required(p, "s3.bucket"),
            normalizePrefix(value(p, "s3.object.key.prefix", "")),
            bool(p, "s3.path.style.access.enabled", false),
            bool(p, "s3.tls.verify", true),
            accessKey,
            secretKey,
            sessionToken,
            positiveLong(p, "s3.multipart.threshold.bytes", 67108864L),
            partSize);

    ApplicationConfig.SecurityMode mode;
    try {
      mode =
          ApplicationConfig.SecurityMode.valueOf(
              required(p, "kafka.security.mode").toUpperCase(Locale.ROOT));
    } catch (IllegalArgumentException e) {
      throw new IllegalArgumentException(
          "kafka.security.mode must be PLAINTEXT, SSL, or SASL_SSL", e);
    }
    ...
    if (mode == ApplicationConfig.SecurityMode.SASL_SSL) {
      ...
      if (jaas.isEmpty()) {
        required(p, "kafka.sasl.username");
        required(p, "kafka.sasl.password");
      }
    }
    String keystore = value(p, "kafka.ssl.keystore.location", "");
    if (!keystore.isEmpty()) requireFile("kafka.ssl.keystore.location", keystore);
    ApplicationConfig.KafkaCommon kc =
        new ApplicationConfig.KafkaCommon(
            required(p, "kafka.bootstrap.servers"),
            mode,
            truststore,
            value(p, "kafka.ssl.truststore.password", ""),
            keystore,
            value(p, "kafka.ssl.keystore.password", ""),
            value(p, "kafka.ssl.key.password", ""),
            value(p, "kafka.ssl.endpoint.identification.algorithm", "https"),
            mechanism,
            value(p, "kafka.sasl.username", ""),
            value(p, "kafka.sasl.password", ""),
            jaas);
    ...
    ApplicationConfig.Producer healthProducer =
        producer(
            p,
            "health",
            deviceId + "-health",
    ...
    ApplicationConfig.Health health =
        new ApplicationConfig.Health(
            bool(p, "health.enabled", true),
            bool(p, "health.required", true),
            positiveLong(p, "health.interval.seconds", 60),
            nonNegativeLong(p, "health.initial.delay.seconds", 10),
            positiveLong(p, "health.worker.stale.threshold.seconds", 180),
            positiveInt(p, "health.init.max.attempts", 3),
            nonNegativeLong(p, "health.init.retry.backoff.ms", 5000),
            positiveInt(p, "health.send.max.consecutive.failures", 5),
            positiveLong(p, "health.send.max.failure.duration.seconds", 300),
            bool(p, "health.recovery.enabled", true),
            positiveInt(p, "health.recovery.max.attempts", 2),
            nonNegativeLong(p, "health.recovery.retry.backoff.ms", 5000),
            positiveLong(p, "health.shutdown.timeout.seconds", 5),
            positiveInt(
                p, "health.fatal.exit.code", positiveInt(p, "application.fatal.exit.code", 20)));
    ApplicationConfig.Identity identity =
        new ApplicationConfig.Identity(
            deviceId,
            deviceName,
            systemId,
            required(p, "health.source.type"),
            programName,
            required(p, "health.program.version"),
    ...
    return new ApplicationConfig(
        value(p, "application.name", "file-collector"),
        value(p, "application.version", "2.0-SNAPSHOT"),
        positiveInt(p, "application.fatal.exit.code", 20),
        fc,
        s3,
        kc,
        fileProducer,
  ...
  private static ApplicationConfig.Producer producer(
      Properties p, String prefix, String clientId, int linger, long buffer) {
    String kafkaPrefix = prefix + ".kafka.";
    String topicKey = "health".equals(prefix) ? "health.topic" : "file.kafka.topic";
    return new ApplicationConfig.Producer(
        required(p, topicKey),
        clientId,
        ...
        positiveInt(
            p, kafkaPrefix + "delivery.timeout.ms", "health".equals(prefix) ? 10000 : 120000),
        positiveInt(p, kafkaPrefix + "max.block.ms", "health".equals(prefix) ? 3000 : 60000),
        linger,
        buffer);
  }
  ...
  private static Set<String> extensions(String raw) {
    ...
    if (result.isEmpty())
      throw new IllegalArgumentException("file.allowed.extensions must not be empty");
    return result;
  }
  ...
  private static String normalizePrefix(String raw) {
    ...
    if (value.contains("../") || value.equals(".."))
      throw new IllegalArgumentException("s3.object.key.prefix must not contain '..'");
    return value;
  }
  ...
  private static void validateDateDirectoryPattern(String pattern) {
    if (pattern.indexOf('Y') >= 0 || pattern.indexOf('D') >= 0)
      throw new IllegalArgumentException(
          "file.watch.date.directory.pattern must use yyyy and dd, not YYYY or DD");
    try {
      String sample =
          DateTimeFormatter.ofPattern(pattern, Locale.ROOT).format(LocalDate.of(2026, 9, 1));
      if (sample.isEmpty()
          || Paths.get(sample).getNameCount() != 1
          || sample.contains("/")
          || sample.contains("\\")
          || ".".equals(sample)
          || "..".equals(sample)) throw new IllegalArgumentException();
    } catch (Exception e) {
      throw new IllegalArgumentException(
  ...
  private static String simpleDirectoryName(String key, String value) {
    if (value.isEmpty()
        ...
        || ".".equals(value)
        || "..".equals(value)
        || Paths.get(value).getNameCount() != 1)
  ...
  private static URI uri(String raw, String key) {
    try {
      ...
      throw new IllegalArgumentException(key + " must be an absolute URI", e);
    }
  }
  ...
  private static String value(Properties p, String key, String d) {
    return p.getProperty(key, d).trim();
  }

  private static boolean bool(Properties p, String key, boolean d) {
    return Boolean.parseBoolean(value(p, key, String.valueOf(d)));
  }

  private static int intValue(Properties p, String key, int d) {
    try {
      return Integer.parseInt(value(p, key, String.valueOf(d)));
    } catch (NumberFormatException e) {
      throw new IllegalArgumentException(key + " must be an integer", e);
    }
  }

  private static long longValue(Properties p, String key, long d) {
    try {
      return Long.parseLong(value(p, key, String.valueOf(d)));
    } catch (NumberFormatException e) {
      throw new IllegalArgumentException(key + " must be an integer", e);
    }
  }

  private static int positiveInt(Properties p, String key, int d) {
    int v = intValue(p, key, d);
    if (v <= 0) throw new IllegalArgumentException(key + " must be > 0");
    return v;
  }

  private static int nonNegativeInt(Properties p, String key, int d) {
    int v = intValue(p, key, d);
    if (v < 0) throw new IllegalArgumentException(key + " must be >= 0");
    return v;
  }

  private static long positiveLong(Properties p, String key, long d) {
    long v = longValue(p, key, d);
    if (v <= 0) throw new IllegalArgumentException(key + " must be > 0");
    return v;
  }

  private static long nonNegativeLong(Properties p, String key, long d) {
    long v = longValue(p, key, d);
    if (v < 0) throw new IllegalArgumentException(key + " must be >= 0");
    return v;
  }
