...
class HealthCloudEventFactoryTest {
  ...
  @Test
  void matchesSampleEnvelopeAndDataFields() throws Exception {
    ...
    CloudEvent event =
        new HealthCloudEventFactory(config, new ObjectMapper(), clock)
            .create(state.snapshot(), new HealthStatus(HealthStatus.Level.UP, "NORMAL", "ok"), 1);
    assertEquals("1.0", event.getSpecVersion().toString());
    assertEquals("healthcheck", event.getSubject());
    assertEquals(config.identity.eventType, event.getType());
    assertEquals("application/json", event.getDataContentType());
    JsonNode data = new ObjectMapper().readTree(event.getData().toBytes());
    assertTrue(data.has("sourceInfo"));
    assertTrue(data.has("status"));
    assertTrue(data.has("heartbeat"));
    assertTrue(data.has("kafkaInfo"));
    assertTrue(data.has("workInfo"));
    assertTrue(data.has("errorInfo"));
  }
}
